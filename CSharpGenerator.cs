...
public class CSharpGenerator : GeneratorCodeAbstract //, ICSharpGenerator
{
    ...
    public void StartClass(int tabCount, AccessModifiers _public, bool _static, string className,
        params string[] derive)
    {
        AddTab(tabCount);
        ...
        {
            sb.AddItem(":");
            for (var i = 0; i < derive.Length - 1; i++) sb.AddItem(derive[i] + ",");
    ...
    public void Field(int tabCount, AccessModifiers _public, bool _static, VariableModifiers variableModifiers,
        string type, string name, bool addHyphensToValue, string value)
    {
        var oio = ObjectInitializationOptions.Original;
        if (addHyphensToValue) oio = ObjectInitializationOptions.Hyphens;
    ...
    public void Field(int tabCount, AccessModifiers _public, bool _static, VariableModifiers variableModifiers,
        string type, string name, ObjectInitializationOptions oio, string value)
    {
        AddTab(tabCount);
    ...
    public void Field(int tabCount, AccessModifiers _public, bool _static, VariableModifiers variableModifiers,
        string type, string name, bool defaultValue)
    {
        AddTab(tabCount);
        ModificatorsField(_public, _static, variableModifiers);
        ReturnTypeName(type, name);
        ...
        string ns)
    {
        ...
        for (var i = 0; i < contentFileNew.Count; i++)
            ...
            {
                if (contentFileNew[i].Contains("{")) lsf = true;
            }
            else if (cl && lsf)
            {
    ...
    private void ModificatorsField(AccessModifiers _public, bool _static, VariableModifiers variableModifiers)
    {
        ...
        else
        {
            ...
            {
                if (_static) sb.AddItem("static");
                if (variableModifiers == VariableModifiers.ReadOnly) sb.AddItem("readonly");
            }
        }
    }
    ...
    /// <param name="args"></param>
    public void Ctor(int tabCount, ModifiersConstructor mc, string ctorName, bool autoAssing, bool isBase,
        params string[] args)
    {
        AddTab(tabCount);
        var sb2 = new StringBuilder(mc.ToString());
    ...
    public void Property(int tabCount, AccessModifiers _public, bool _static, string returnType, string name,
        object _get, object _set, string field, bool shortGet, bool shortSet)
    {
        #region MyRegion
        ...
        if (settedSet)
        {
            if (shortSet) throw new Exception("Can't be set shortSet and _set in one time");
        ...
        if (shortGet)
        {
            if (settedSet) throw new Exception("Can't be set shortGet and _get in one time");
            sb.AddItem("get;");
        }

        if (shortSet)
        {
            if (settedGet) throw new Exception("Can't be set shortGet and _get in one time");
            sb.AddItem("set;");
        }
    ...
    /// <param name="inner"></param>
    /// <param name="args"></param>
    public void Method(int tabCount, AccessModifiers _public, bool _static, string returnType, string name,
        string inner, string args)
    {
        AddTab(tabCount);
        PublicStatic(_public, _static);
        ReturnTypeName(returnType, name);
    ...
    public void EnumWithComments(int tabCount, AccessModifiers _public, string nameEnum,
        Dictionary<string, string> nameCommentEnums)
    {
        WriteAccessModifiers(_public);
    ...
    {
        var zav = "";
        if (inParentheses != null) zav = "(" + inParentheses + ")";
    ...
    public void AssignValue(int tabCount, string objectName, string variable, string value, CSharpGeneratorArgs a)
    {
        ...
        sb.AddItem("=");
        if (a.addHyphens) value = SH.WrapWith(value, "\"");
    ...
    /// <param name="a"></param>
    public void AddValuesViaAddRange(int tabCount, string timeObjectName, string v, string type,
        List<string> whereIsUsed2, CSharpGeneratorArgs a)
    {
        var objectIdentificator = "";
        if (timeObjectName != null) objectIdentificator = timeObjectName + ".";
        if (a.addHyphens)
    ...
    public void AddValuesViaAddRange(int tabCount, string timeObjectName, string v, Type type,
        List<string> whereIsUsed2, CSharpGeneratorArgs a)
    {
    ...
    public void DictionaryStringListString(int tabCount, string nameDictionary, Dictionary<string, List<string>> result,
        CSharpGeneratorArgs a = null)
    {
        if (a == null) a = new CSharpGeneratorArgs();
    ...
    public void DictionaryFromRandomValue<Key, Value>(int tabCount, string nameDictionary, List<Key> keys,
        Func<Value> randomValue, CSharpGeneratorArgs a = null)
    {
        if (a == null)
        {
        ...
        for (var i = 0; i < keys.Count; i++) dict.Add(keys[i], randomValue.Invoke());
    ...
    /// <param name="addingValue"></param>
    public void DictionaryFromTwoList<Key, Value>(int tabCount, string nameDictionary, List<Key> keys,
        List<Value> values, CSharpGeneratorArgs a = null)
    {
        ...
        var dict = new Dictionary<Key, Value>();
        for (var i = 0; i < keys.Count; i++) dict.Add(keys[i], values[i]);
    ...
    /// <param name="addingValue"></param>
    public void DictionaryFromDictionary<Key, Value>(int tabCount, string nameDictionary, Dictionary<Key, Value> dict,
        CSharpGeneratorArgs arg = null)
    {
        if (arg == null) arg = new CSharpGeneratorArgs();

        string valueType = null;
        if (dict.Count > 0)
            valueType = ConvertTypeShortcutFullName.ToShortcut(DictionaryHelper.GetFirstItemValue(dict).GetType()
                .FullName);
        ...
        if (arg.addingValue) GetDictionaryValuesFromDictionary(tabCount, nameDictionary, dict);
    }
    ...
    /// <param name="dict"></param>
    /// <param name="addingValue"></param>
    public void DictionaryFromDictionaryInnerList<Key, Value>(int tabCount, string nameDictionary,
        Dictionary<Key, Value> dict, CSharpGeneratorArgs a)
    {
        string valueType = null;
        if (dict.Count > 0)
            valueType = ConvertTypeShortcutFullName.ToShortcut(DictionaryHelper.GetFirstItemValue(dict).GetType()
        ...
        if (a.addingValue) GetDictionaryValuesFromDictionary(tabCount, nameDictionary, dict);
    }

    public void GetDictionaryValuesFromRandomValue<Key, Value>(int tabCount, string nameDictionary, List<Key> keys,
        Func<Value> randomValue)
    {
        var dict = new Dictionary<Key, Value>();
        for (var i = 0; i < keys.Count; i++) dict.Add(keys[i], randomValue.Invoke());
    ...
    /// <param name="values"></param>
    /// <param name="a"></param>
    public void GetDictionaryValuesFromTwoList<Key, Value>(int tabCount, string nameDictionary, List<Key> keys,
    ...
    public void GetDictionaryValuesFromDictionaryInnerList<Key, Value>(int tabCount, string nameDictionary,
        Dictionary<Key, List<Value>> dict, CSharpGeneratorArgs a)
    {
        Type tKey, tValue;
        ...
        if (a.alsoField)
            Field(tabCount, AccessModifiers.Public, true, VariableModifiers.None,
                string.Format("Dictionary<{0}, List<{1}>>", tKey.Name, tValue.Name), nameDictionary, true);
    ...
    public static void GetTKeyAndTValue<Key, Value>(Dictionary<Key, List<Value>> dict, out Type tKey, out Type tValue,
        out Key key)
    {
        key = default;
    ...
    public void GetDictionaryValuesFromDictionary<Key, Value>(int tabCount, string nameDictionary,
